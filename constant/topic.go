...
const (
	...
	TopicDevicePropertyReportReply = TopicPrefix + "%s/thing/property/post_reply" //平台->设备 属性上报响应

	// 事件上报
	TopicDeviceEventReport      = TopicPrefix + "+/+/thing/event/post"      //设备->平台 设备事件上报
	TopicDeviceEventReportReply = TopicPrefix + "%s/thing/event/post_reply" //平台->设备 事件上报响应

	// 设置属性
	TopicDevicePropertySetReply = TopicPrefix + "+/+/thing/property/set_reply" //设备->平台 设置设备属性响应
	TopicDevicePropertySet      = TopicPrefix + "%s/thing/property/set"        //平台->设备 设置设备属性

	// 设备属性查询
	TopicDevicePropertyQueryReply = TopicPrefix + "+/+/thing/property/query_reply" //设备->平台 设备属性查询响应
	TopicDevicePropertyQuery      = TopicPrefix + "%s/thing/property/query"        //平台->设备 设备属性查询

	// 设备服务调用
	TopicDeviceServiceInvokeReply = TopicPrefix + "+/+/thing/service/invoke_reply" //设备->平台 设备服务调用响应
	TopicDeviceServiceInvoke      = TopicPrefix + "%s/thing/service/invoke"        //平台->设备 设备服务调用
	...
	TopicSubDevicePropertyReportReply = TopicPrefix + "%s/thing/sub/property/post_reply" //平台->设备 子设备属性上报响应

	//子设备事件上报
	TopicSubDeviceEventReport      = TopicPrefix + "+/+/thing/sub/event/post"      //设备->平台 设备事件上报
	TopicSubDeviceEventReportReply = TopicPrefix + "%s/thing/sub/event/post_reply" //平台->设备 事件上报响应

	//子设备设置属性
	TopicSubDevicePropertySetReply = TopicPrefix + "+/+/thing/sub/property/set_reply" //设备->平台 设置设备属性响应
	TopicSubDevicePropertySet      = TopicPrefix + "%s/thing/sub/property/set"        //平台->设备 设置设备属性

	//子设备设备属性查询
	TopicSubDevicePropertyQueryReply = TopicPrefix + "+/+/thing/sub/property/query_reply" //设备->平台 设备属性查询响应
	TopicSubDevicePropertyQuery      = TopicPrefix + "%s/thing/sub/property/query"        //平台->设备 设备属性查询
